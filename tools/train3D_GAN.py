...
from core.function3D_GAN import train
from core.function3D_GAN import validate
...
from models.triangulation import RANSACTriangulationNet, AlgebraicTriangulationNet, VolumetricTriangulationNet, VolumetricTriangulationNet_CPM, Discriminator
...
def main_worker(gpus, ngpus_per_node, args, final_output_dir, tb_log_dir):
    ...
    D_optimizer = torch.optim.RMSprop([{'params': filter(lambda p: p.requires_grad, discriminator.parameters()), 'initial_lr': cfg.TRAIN.LR}], lr=cfg.TRAIN.LR)
    ...
    if cfg.DISTRIBUTED: # This block is not available
        ...
        discriminator = torch.nn.DataParallel(discriminator, gpu_ids).cuda(device)
    ...
    with torch.autograd.set_detect_anomaly(True):
        for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
            ...
            train(epoch, cfg, args, master, train_loader_dict, [model, discriminator], criterion, [optimizer, D_optimizer],
            ...
            if not cfg.WITHOUT_EVAL:
                ...
                with torch.no_grad():
                    recorder = validate(
                        cfg, args, master, valid_loader_dict, [model, discriminator], criterion,
...
def save_checkpoint(states, is_best, output_dir,
    ...
    if is_best:
        # if not hasattr(save_checkpoint,'last_best_epoch'):   
        #     save_checkpoint.last_best_epoch = states['epoch']
        # else:
        #     os.remove(os.path.join(output_dir, 'best_state_epoch{}.pth.tar'.format(save_checkpoint.last_best_epoch)))
        ...
        with open(os.path.join(output_dir, 'best_epoch_{}.txt'.format(states['epoch'])), 'w') as f:
            pass
        #save_checkpoint.last_best_epoch  = states['epoch']
