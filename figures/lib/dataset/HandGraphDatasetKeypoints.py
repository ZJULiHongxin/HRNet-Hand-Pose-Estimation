...
class HandGraphDataset_Keypoint(HandGraphDataset):
    ...
    def __getitem__(self, idx):
        img, pose_2d, local_pose3d_gt, mesh_2d, local_mesh_pts_gt, mesh_tri_idx, cam_proj_mat, img_path = super().__getitem__(idx)
        pose_mesh_2d = np.vstack((pose_2d, mesh_2d)) # (21+N) x 3

        # for training and validation; No data augmentation yet !
        # img_rgb: torch.tensor of size 3 x 256 x 256; # pose_mesh_2d is a list containing only one element which is a 
        # np.array of size (21+N)x3. The 3rd column stands for visibility
        img_rgb, pose_mesh_2d = self.transforms( 
            img[:,:,0:3], [pose_mesh_2d]         
        )

        pose_2d = pose_mesh_2d[0][0:pose_2d.shape[0], :]
        mesh_2d = pose_mesh_2d[0][pose_2d.shape[0]:, 0:-1]
        ...
        target_heatmaps = self.heatmap_generator(pose_2d) 

        # numpy array of size 256 x 256. Elements are either 255 or 0
        img_mask = cv2.resize(img[:,:,3], tuple(self.config.MODEL.IMAGE_SIZE))
        
        return img_rgb, img_mask, \
                target_heatmaps, \
                pose_2d, \
    ...
    def visualize_data(self):
        for i in range(self.__len__()):
            # img_rgb type: float32; img_mask type uint8
            img_rgb, img_mask, target_heatmaps, pose_2d, local_pose3d_gt, mesh_2d, local_mesh_pts_gt, mesh_tri_idx, cam_proj_mat = self.__getitem__(i)
            img_rgb = cv2.cvtColor(np.transpose(img_rgb.numpy(),(1,2,0)), cv2.COLOR_RGB2BGR)
            ...
            factor = self.config.MODEL.IMAGE_SIZE[0] / self.config.MODEL.HEATMAP_SIZE[0]
            skeleton_overlay = draw_2d_skeleton(img_mask, factor * pose_2d[:,0:2])
