...
class HandGraphDataset(Dataset):
    ...
    def __init__(self, root, set_name, data_format, transform=None,
        ...
        self.name = 'HandGrpah'
        self.root = root
        self.data_dir = os.path.join(root, set_name) # HandGraph/training 
        self.set_name = set_name
        self.transform = transform
        self.target_transform = target_transform

        camera_param_path = osp.join(self.root,'3D_labels/camPosition.txt')
        global_pose3d_gt_path = osp.join(self.root, '3D_labels/handGestures.txt')
        
        val_set_path = osp.join(self.root, '3D_labels/val-camera.txt')

        self.image_dir = osp.join(self.root, 'images')
        self.global_mesh_gt_dir = osp.join(self.root, 'hand_3D_mesh')
        ...
        legend_dict = OrderedDict(sorted(zip(std_legend_lst,idx_HandGraph), key=lambda x:x[1]))
        self.joint_label_list = list(legend_dict.keys())
        
        logger.info(self.__repr__())
        logger.info('=> classes: {}'.format(list(self.joint_indices)))
    ...
    def __getitem__(self, idx):
        ...
        img = cv2.imread(img_path, cv2.IMREAD_UNCHANGED) # RGBA
        ...
        pose_2d = cam_projection(local_pose3d_gt, cam_proj_mat) # (N, 2)
        mesh_2d = cam_projection(local_mesh_pts_gt, cam_proj_mat)

        # append a column representing visibility
        pose_2d = np.concatenate((pose_2d, np.ones((21,1))), axis=1)
        mesh_2d = np.concatenate((mesh_2d, np.ones((mesh_2d.shape[0],1))), axis=1)
        
        if self.transform is not None:
            img, joints_list = self.transform(img,[pose_2d])
            return img, joints_list[0], img_path

        if self.target_transform is not None:
            pose_2d = self.target_transform(pose_2d)
        
        return img, pose_2d, local_pose3d_gt, mesh_2d, local_mesh_pts_gt, mesh_tri_idx, cam_proj_mat, img_path
    ...
    def visualize_data(self):
        for i in range(self.__len__()):
            img, pose_2d, local_pose3d_gt, mesh_2d, local_mesh_pts_gt, mesh_tri_idx = self.__getitem__(i)
            img_rgb = img[:, :, :3]
            img_mask = img[:, :, 3:]
