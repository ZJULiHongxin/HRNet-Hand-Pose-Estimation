...
def transform_global_to_cam(global_3d, camera_param, use_translation=True):
    ...
    rot_mx = euler_xyz_to_rot_mx(theta) # (3,3)
...
def cam_deprojection(pose_2d, cam_proj_mat, z=1.0):
    """
    get 3D coords from 2D projection points
    :param pose_2d: (N, 2)
    :param cam_proj_mat: (3, 3)
    :param z: reference depth
    :return:
    """
    pose_hetero2d = z * np.hstack((pose_2d, np.ones((pose_2d.shape[0],1), dtype=pose_2d.dtype)))
    cam_mat_inv = np.linalg.inv(cam_proj_mat.transpose())
    return pose_hetero2d.dot(cam_mat_inv)
        ...
        for line in reader:
            ...
            try:
                if fields[0] == 'v':
                    if state != 'V':
                        break
                    mesh_pts.append([float(f) for f in fields[1:]]) # ex: v 8.611394 -0.569481 2.183960
                if fields[0] == 'f':
                    state = 'F'
                    mesh_tri_idx.append([int(f.split('/')[0]) - 1 for f in fields[1:]]) # example: f 741/832/3955 740/830/3956 752/844/3957
                if fields[0] == 'vn':
                    state = 'N'
                    if id_vn % 3 == 0:
