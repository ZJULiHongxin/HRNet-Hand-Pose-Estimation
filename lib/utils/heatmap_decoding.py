...
def get_final_preds(hms, use_softmax=True):
    """
    params:
    ...
    assert hms.ndim == 4, 'Heatmap shape should be 4-ndim'

    if use_softmax:
        preds = kornia.geometry.subpix.spatial_expectation2d(hms, normalized_coordinates=False)
        return preds
    else:
        batch_size, num_joints, hm_size = hms.shape[0:3]

        pose2d_pred_u =  torch.argmax(hms.view((batch_size, num_joints,-1)),dim=2)
        pose2d_pred_u, pose2d_pred_v = pose2d_pred_u % hm_size, pose2d_pred_u // hm_size
