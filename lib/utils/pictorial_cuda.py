...
def infer(unary, pairwise, body, config):
    ...
    for node in skeleton_sorted_by_level:
        children_state = []
        u = unary[node['idx']].clone() # 1 x 16*16*16
        if len(node['children']) == 0:
            energy = u # 1 x 16*16*16
...
def compute_grid(boxSize, boxCenter, nBins, device=None):
    ...
    gridx = gridx.contiguous().view(-1, 1) # 16 x 16 x 16
    gridy = gridy.contiguous().view(-1, 1)
    gridz = gridz.contiguous().view(-1, 1)
    grid = torch.cat([gridx, gridy, gridz], dim=1) # 4096 x 3
    return grid
    ...
    for v in range(n):
        ...
        for j in range(k):
            ...
            if (share_grid and j == 0) or not share_grid:
                xy = cameras.project_pose(grid[grid_id], cam[v]) # world -> image
                xy = do_transform(xy, trans) * torch.tensor( # do affine transformation and scale the result to the heatmap size
...
def rpsm(cams, heatmaps, config, kw):
    ...
    # all in this device
    device = heatmaps.device
    img_size = config.NETWORK.IMAGE_SIZE # 320
    map_size = config.NETWORK.HEATMAP_SIZE 
    grd_size = config.PICT_STRUCT.GRID_SIZE # 2000 mm
    fst_nbins = config.PICT_STRUCT.FIRST_NBINS # 16
    rec_nbins = config.PICT_STRUCT.RECUR_NBINS # 2
    rec_depth = config.PICT_STRUCT.RECUR_DEPTH # 10
    tolerance = config.PICT_STRUCT.LIMB_LENGTH_TOLERANCE # 150

    # kw['center']: the root (pelvis) of human skeleton in the world coordinate system
    grid = compute_grid(grd_size, kw['center'], fst_nbins, device=device) # 4096 x 3
    unary = compute_unary_term(heatmaps, [grid], kw['boxes'], cams, img_size) # a list of unary values for each joint
