...
def train(epoch, config, args, master, train_loader_dict, model, criterion, optimizer,
    ...
    model.train()
    discriminator.train()
    optimizer, D_optimizer = optimizer
    ...
    for dataset_name, train_loader in train_loader_dict.items():
        ...
        for i, ret in enumerate(train_loader):
            ...
            if config.LOSS.WITH_KCS_LOSS:
                ...
                item_dict['data_idx'] = ret['data_idx'] # (b,)
                item_dict['discriminator'] = discriminator
                item_dict['train_critic'] = False if i % n_critic == 0 else True
            ...
            if i % n_critic != 0:  # train the discriminator
                D_optimizer.step()
    ...
    for dataset_name, val_loader in val_loader_dict.items():
        ...
        for i,  ret in enumerate(val_loader):
            ...
            if config.LOSS.WITH_KCS_LOSS:
                ...
                item_dict['data_idx'] = ret['data_idx'] # (b,)
                item_dict['discriminator'] = discriminator
                item_dict['train_critic'] = False if i % config.MODEL.N_CRITIC == 0 else True
...
class AverageMeter(object):
    ...
    def computeLosses(self, item_dict, n=1):
        ...
        if 'KCS_loss' in loss_functions and 'KCS_pred' in items:
            ...
            pose3d_gt, pose3d_pred, data_idx = item_dict['pose3d_gt'], item_dict['pose3d_pred'], item_dict['data_idx']
            
            n_batch, split_idx, KCS_loss = pose3d_gt.shape[0], 1, torch.tensor(0., device=KCS_pred.device)
            discriminator, train_critic = item_dict['discriminator'], item_dict['train_critic']

            def calc_critic(KCS_gt, KCS_pred, start, end):
                KCS_pred_last_frame = KCS_pred[start:end].clone()
                KCS_gt_last_frame = KCS_gt[start:end]

                KCS_TC_pred = torch.cat((
                    torch.zeros((1,20,20), dtype=KCS_pred.dtype).to(KCS_pred.device),
                    #KCS_pred_last_frame[0:1],
                    KCS_pred[start+1:end+1] - KCS_pred_last_frame,
                    ), dim=0) # b x 20 x 20
                
                seq_len = end - start + 1
                pose3d_KCS_KCST_pred = torch.cat((
                    pose3d_pred[start:end + 1].view(seq_len, -1),
                    KCS_pred[start:end + 1].view(seq_len, -1),
                    KCS_TC_pred.view(seq_len, -1)), dim=1) # b x (21*3 + 20*20 + 20*20)
                #print('[Function]', start, end)
                #print('[Function]', pose3d_KCS_KCST_pred.shape, pose3d_pred.shape, KCS_pred.shape, KCS_TC_pred.shape)
                # GAN loss
                if train_critic:
                    KCS_TC_gt = torch.cat((
                    torch.zeros((1,20,20), dtype=KCS_pred.dtype).to(KCS_pred.device),
                    #KCS_gt_last_frame[0:1],
                    KCS_gt[start+1:end+1] - KCS_gt_last_frame
                    ), dim=0) # b x 20 x 20

                    pose3d_KCS_KCST_gt = torch.cat((
                    pose3d_gt[start:end + 1].view(seq_len, -1),
                    KCS_gt[start:end + 1].view(seq_len, -1),
                    KCS_TC_gt.view(seq_len, -1)), dim=1) # b x (21*3 + 20*20 + 20*20)
                    #print('[Function]', pose3d_KCS_KCST_gt.shape, pose3d_KCS_KCST_pred.shape)
                    return -torch.mean(discriminator(pose3d_KCS_KCST_gt)) + torch.mean(discriminator(pose3d_KCS_KCST_pred.detach()))
                else:
                    return -torch.mean(discriminator(pose3d_KCS_KCST_pred))
            ...
            if split_idx >= 2: 
                KCS_loss += calc_critic(KCS_gt, KCS_pred, start=0, end=split_idx-1)
            
            if data_idx.shape[0] - split_idx >= 2:
                KCS_loss += calc_critic(KCS_gt, KCS_pred, start=split_idx, end=n_batch-1)
