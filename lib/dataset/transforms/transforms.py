...
class ToTensor(object):
    def __call__(self, image, joints):
        return F.to_tensor(image), joints  # to_tensor switches the channel dimension to the 0th dim
...
class RandomHorizontalFlip(object):
    def __init__(self, flip_index, output_size, prob=0.5):
        ...
        self.flip_index = flip_index # a list
        self.prob = prob
...
class RandomAffineTransform(object):
    def __init__(self,
        ...
        # output_size denotes the size of heatmaps, and is used to scale 2D pose groundtruths
        self.name = 'RandomAffineTransform'
    ...
    def _affine_joints(self, joints, mat):
        ...
        return np.dot(np.concatenate(
            (joints, joints[:, 0:1]*0+1), axis=1), # heterogeneous coordinates
             mat.T).reshape(shape)

    def __call__(self, image, joints):
        """
        - joints: is a list object whose length is equal to the number of final deconvolution modules corresponding to different resolutions
        """
        assert isinstance(joints, list)
...
# class ToTensor(object):
#     def __call__(self, image, joints):
#         return F.to_tensor(image), joints
