...
class FreiHandDataset(Dataset):
    ...
    def __init__(self, root, set_name, data_format, transform=None,
        ...
        self.data_dir = os.path.join(root, self.name) # ../data/FreiHand
        split = 0.8
        if set_name in ['train', 'training']:
            self.sample_lst = range(0,int(32560*split))
        elif set_name in ['eval', 'valid', 'val', 'evaluation', 'validation']:
            self.sample_lst = range(int(32560*split),32560)
        
        self.transform = transform
        self.target_transform = target_transform
        self.db_data_anno = list(load_db_annotation(base_path=self.data_dir, set_name='training'))
