...
class HamburgerV1(nn.Module):
    def __init__(self, config, in_c):
        super().__init__()

        ham_type = config.MODEL.HAM_TYPE#getattr(args, 'HAM_TYPE', 'NMF')

        D = config.MODEL.EMB_DIM#getattr(args, 'MD_D', 512)
...
class HamburgerV2(nn.Module):
    def __init__(self, config, in_c):
        ...
        C = config.MODEL.EMB_DIM#getattr(args, 'MD_D', 512)
...
class HamburgerV2Plus(nn.Module):
    def __init__(self, config, in_c):
        ...
        S = config.MODEL.S#getattr(args, 'MD_S', 1)
        D = config.MODEL.EMB_DIM#getattr(args, 'MD_D', 512)
        C = S * D

        self.dual = config.MODEL.DUAL_HAM#getattr(args, 'DUAL', True)
        if self.dual:
            C = 2 * C
        ...
        HAM = get_hams(ham_type)
        if self.dual:
            self.ham_1 = HAM(config, spatial=True)
            self.ham_2 = HAM(config, spatial=False)
        else:
            self.ham = HAM(config)

        factor = config.MODEL.CHEESE_FACTOR#getattr(args, 'CHEESE_FACTOR', S)
        if self.dual:
            factor = 2 * factor
        ...
        zero_ham = config.MODEL.ZERO_HAM#getattr(args, 'ZERO_HAM', True)
        if zero_ham:
            coef_ham_init = 0.
        else:
            coef_ham_init = 1.
    ...
    def forward(self, x):
        ...
        x = self.lower_bread(x) # b x c x h x w

        if self.dual:
            x = x.view(x.shape[0], 2, x.shape[1] // 2, *x.shape[2:]) # b x 2 x c/2 x h x w
