...
class _MatrixDecomposition2DBase(nn.Module):
    def __init__(self, config, spatial=None):
        super().__init__()

        self.spatial = config.MODEL.SPATIAL if spatial is None else spatial #getattr(args, 'SPATIAL', True)

        self.S = config.MODEL.S#getattr(args, 'MD_S', 1)
        self.D = config.MODEL.EMB_DIM#getattr(args, 'MD_D', 512)
        self.R = config.MODEL.R#getattr(args, 'MD_R', 64)

        self.train_steps = config.MODEL.TRAIN_STEPS#getattr(args, 'TRAIN_STEPS', 6)
        self.eval_steps  = config.MODEL.EVAL_STEPS#getattr(args, 'EVAL_STEPS', 7)

        self.inv_t = config.MODEL.INV_T#getattr(args, 'INV_T', 100)
        self.eta   = config.MODEL.ETA#getattr(args, 'ETA', 0.9)

        self.rand_init = config.MODEL.RAND_INIT#getattr(args, 'RAND_INIT', True)
    ...
    def forward(self, x, return_bases=False):
        ...
        if not self.rand_init and not hasattr(self, 'bases'):
            bases = self._build_bases(1, self.S, D, self.R, device=device)
            self.register_buffer('bases', bases)

        # (S, D, R) -> (B * S, D, R)
        if self.rand_init:
            bases = self._build_bases(B, self.S, D, self.R, device=device)
        else:
...
class CD2D(_MatrixDecomposition2DBase):
    def __init__(self, config, spatial=None):
        ...
        self.beta = config.MODEL.BETA#getattr(args, 'BETA', 0.1)
