...
def triangulate_point_from_multiple_views_linear(proj_matricies, points):
    ...
    Args:
        proj_matricies numpy array of shape (N_views, 3, 4): sequence of projection matricies (3x4)
        points numpy array of shape (N_views, 2): sequence of points' coordinates
    ...
    # the first three elements (X,Y,Z) are divided by the fouth element (scaling factor)
...
def triangulate_point_from_multiple_views_linear_torch(proj_matricies, points, confidences=None):
    ...
    Args:
        ...
        confidences None or torch tensor of shape (N_views,): confidences of points [0.0, 1.0].
...
def triangulate_batch_of_points(proj_matricies_batch, points_batch, confidences_batch=None):
    ...
    # points_batch: b x v x 21 x 2
    # confidences_batch: b x N_views x N_joints
...
def calc_reprojection_error_matrix(keypoints_3d, keypoints_2d_list, proj_matricies):
    ...
    for keypoints_2d, proj_matrix in zip(keypoints_2d_list, proj_matricies): # for each view
        keypoints_2d_projected = project_3d_points_to_image_plane_without_distortion(proj_matrix, keypoints_3d)
        reprojection_error = 1 / 2 * np.sqrt(np.sum((keypoints_2d - keypoints_2d_projected) ** 2, axis=1)) # (21,)
        reprojection_error_matrix.append(reprojection_error)

    return np.vstack(reprojection_error_matrix).T # N_points x N_views
