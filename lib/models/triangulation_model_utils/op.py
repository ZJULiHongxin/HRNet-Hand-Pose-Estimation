...
from .img import to_numpy, to_torch
...
def unproject_heatmaps(heatmaps, proj_matricies, coord_volumes, volume_aggregation_method='sum', vol_confidences=None):
    # coord_volumes (after rotation augmentation): batch_size x 64 x 64 x 64 x 3 unit: mm
    device = heatmaps.device
    batch_size, n_views, n_joints, heatmap_shape = heatmaps.shape[0], heatmaps.shape[1], heatmaps.shape[2], tuple(heatmaps.shape[3:])
    volume_shape = coord_volumes.shape[1:4] # 64 x 64 x 64
    ...
    for batch_i in range(batch_size):
        ...
        for view_i in range(n_views):
            ...
            # grid_sample gets a higher-resolution tensor from the input tensor by using interpolation
