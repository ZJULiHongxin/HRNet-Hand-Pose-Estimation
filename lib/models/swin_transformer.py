...
from . import pose_hrnet_softmax
...
class LocalityFeedForward(nn.Module):
    def __init__(self, in_features, out_features, stride, expand_ratio=4., act='hs+se', reduction=4,
    ...
    def forward(self, x):
        # x: b x n x d
        batch_size, n_patches, vec_dim = x.shape
        x = x.transpose(1,2).contiguous().view(batch_size, vec_dim, int(n_patches ** 0.5), int(n_patches ** 0.5))
        x = x + self.conv(x)
...
class SwinTransformerBlock(nn.Module):
    ...
    Args:
        ...
        expand_ratio (float): Ratio of mlp hidden dim to embedding dim.
        qkv_bias (bool, optional): If True, add a learnable bias to query, key, value. Default: True
        qk_scale (float | None, optional): Override default qk scale of head_dim ** -0.5 if set.
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
                 expand_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 feed_forward='mlp', norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        if feed_forward == 'mlp':
            self.feed_forward = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=nn.GELU, drop=drop)
        elif feed_forward == 'conv':
            self.feed_forward = LocalityFeedForward(in_features=dim, out_features=dim, stride=1, expand_ratio=expand_ratio)
        self.H = None
        self.W = None
...
class BasicLayer(nn.Module):
    ...
    Args:
        ...
        expand_ratio (float): Ratio of mlp hidden dim to embedding dim. Default: 4.
        qkv_bias (bool, optional): If True, add a learnable bias to query, key, value. Default: True
        qk_scale (float | None, optional): Override default qk scale of head_dim ** -0.5 if set.
        drop (float, optional): Dropout rate. Default: 0.0
        attn_drop (float, optional): Attention dropout rate. Default: 0.0
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.norm is not None:
            ...
            x = x.flatten(2).transpose(1, 2) # b x Wh*Ww x C
            x = self.norm(x)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        out_indices = list(range(len(depths)))
...
class SwinPose(nn.Module):
    def __init__(self, config):
        ...
        self.swinTransformer = SwinTransformer(
            ...
            ape=config.MODEL.ABSOLUTE_POSITION_ENCODING,
            feed_forward=config.MODEL.FF_TYPE
        )

        fmt_str = 'Model info: ' + SwinTransformer.__name__ + '\n'
        fmt_str += '    input_size: {}\n'.format(input_size)
        fmt_str += '    patch_size: {}\n'.format(config.MODEL.PATCH_SIZE)
        fmt_str += '    embed_dim: {}\n'.format(config.MODEL.EMB_DIM)
        fmt_str += '    depths: {}\n'.format(config.MODEL.DEPTHS)
        fmt_str += '    absolute pe: {}\n'.format(config.MODEL.ABSOLUTE_POSITION_ENCODING)
        fmt_str += '    feed_forward: {}\n'.format(config.MODEL.FF_TYPE)
        print(fmt_str)

        decoder = []
        
        expand_ratio = 2 ** (len(config.MODEL.DEPTHS) - 1)
        hmsize_vs_outsize = config.MODEL.HEATMAP_SIZE[0] // (input_size // config.MODEL.PATCH_SIZE // expand_ratio) # 64 / 8 = 8
        for i in range(int(math.log2(hmsize_vs_outsize))): # 8, 8C; 16,4C; 32, 2C; 64, C; 64,21
            in_channel, out_channel = config.MODEL.EMB_DIM * expand_ratio // 2 ** i, config.MODEL.EMB_DIM * expand_ratio // 2 ** (i + 1)
            decoder.extend(
                [
        ...
        decoder.append(nn.Conv2d(
                        in_channels=decoder[-3].out_channels,
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'backbone'):
            _, x, _ = self.backbone(x)
        
        feat_all_stages = self.swinTransformer(x) # a tuple
        x = self.decoder(feat_all_stages[-1]) # b x 21 x H x W
    ...
    def train(self, mode: bool):
        self.swinTransformer.train()
        return super().train(mode=mode)

def get_pose_net(cfg, is_train, **kwargs):
    model = SwinPose(cfg, **kwargs)
